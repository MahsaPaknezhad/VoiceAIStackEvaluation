...
class PipelineExecutor:
    ...
    def _prepare_batch_stt(
        ...
        execution. Records STT timing on the timing collector so batch
        inference latency is captured. Streaming services are unaffected.

        Args:
            stt_service: STT service instance that may support batch
            transcription audio_path: Path to audio file for batch
            transcription
            pipeline_components: Pipeline components containing timing
            collector
        """
        if hasattr(stt_service, 'transcribe_file'):
            logger.info(f"Using batch transcription for Whisper: {audio_path}")
            timing = pipeline_components.collectors.timing
            timing.stt_start_time = time.time()
            transcription = stt_service.transcribe_file(audio_path)
            timing.stt_end_time = time.time()
            logger.info(
                f"Whisper transcription result: '{transcription}' "
                f"({timing.get_stt_latency_ms():.1f}ms)"
            )
    ...
    async def execute_pipeline(
        ...
        Returns:
            ...
            audio
        """
        self._prepare_batch_stt(stt_service, audio_path, pipeline_components)
