...
class STTServiceFactory(BaseServiceFactory):
    ...
    def _create_standard_service(self, config: Dict[str, Any]) -> Any:
        ...
        # Use native WhisperSTTService with Settings API for real-time evaluation
        if config["class"] == "WhisperSTTService":
            logger.info(
                "Using native WhisperSTTService for real-time evaluation"
            )
            whisper_config = config.get("config", {})
            settings = service_class.Settings(
                model=whisper_config.get("model"),
                language=whisper_config.get("language"),
                no_speech_prob=whisper_config.get("no_speech_prob", 0.2),
            )
