...
def extract_metrics(results: List[Dict]) -> Tuple[Dict, Dict, Dict, Dict]:
    ...
    for result in results:
        ...
        for eval_item in result.get('evaluations', []):
            ...
            voice_quality = eval_item.get('voice_quality', {}) or {}
            if (voice_quality.get('fluency') or {}).get('score') is not None:
                fluency.append(voice_quality['fluency']['score'])
            if (voice_quality.get('tone') or {}).get('score') is not None:
                tone.append(voice_quality['tone']['score'])
            if (voice_quality.get('naturalness') or {}).get('score') is not None:
        ...
        if stt_latencies and wers:
            ...
            stt_metrics[stt_model]['n'].append(len(wers))
    ...
    for model in stt_metrics:
        total_n = sum(stt_metrics[model]['n']) if stt_metrics[model]['n'] else 1
        stt_metrics[model] = {
...
def plot_stt_metrics(stt_metrics: Dict, output_path: str):
    """Scientific plot of STT performance, separate API and local PNGs."""
    ...
    provider_styles = {
        ...
        'gladia': {'color': '#bcbd22', 'marker': '<', 'label': 'Gladia'},
    }
    
    LOCAL_THRESHOLD = 100
    api_models = {m: v for m, v in filtered.items() if v['latency'] >= LOCAL_THRESHOLD}
    local_models = {m: v for m, v in filtered.items() if v['latency'] < LOCAL_THRESHOLD}
    
    def get_provider(model):
        if 'whisper' in model.lower(): return 'whisper'
        if 'aws' in model.lower() or 'transcribe' in model.lower(): return 'aws'
        if 'deepgram' in model.lower(): return 'deepgram'
        if 'assemblyai' in model.lower(): return 'assemblyai'
        if 'nvidia' in model.lower() or 'parakeet' in model.lower(): return 'nvidia'
        if 'gladia' in model.lower(): return 'gladia'
        return 'aws'
    
    base = output_path.replace('.png', '')
    
    # --- Single combined plot with broken axis ---
    
    # Per-model label offsets to avoid overlap
    label_offsets = {
        'nvidia_parakeet': (-50, 25),
        'whisper_small': (15, 25),
        'whisper_turbo': (15, -20),
        'whisper_large_v2': (15, -40),
        'assemblyai': (15, 12),
        'aws_transcribe': (15, -25),
        'deepgram_nova2': (-70, -20),
        'deepgram_nova3': (-70, 15),
    }
    
    all_models = {**api_models, **local_models}
    
    # Broken axis: left panel (local: 0.05–2ms), right panel (API: 500–15000ms)
    fig, (ax_left, ax_right) = plt.subplots(1, 2, figsize=(14, 8), sharey=True,
                                             gridspec_kw={'width_ratios': [1, 1.5], 'wspace': 0.05})
    
    for ax_obj, xlim in [(ax_left, (0.15, 0.6)), (ax_right, (2000, 12000))]:
        plotted_providers_ax = set()
        for i, (model, metrics) in enumerate(sorted(all_models.items())):
            ...
            # Skip if point not in this panel's range
            if x < xlim[0] * 0.5 or x > xlim[1] * 2:
                continue
            
            # Colored vertical band for WER ±1 SD
            bw = x * 0.06 if x > 1 else 0.01
            ax_obj.fill_between([x - bw, x + bw], y - y_std, y + y_std,
                               color=style['color'], alpha=0.15, zorder=1)
            
            # Arrow-style horizontal error bars for latency ±1 SD
            x_right = min(x + x_std, xlim[1] * 0.95)
            x_left = max(x - x_std, xlim[0] * 1.05)
            ax_obj.annotate('', xy=(x_right, y), xytext=(x, y),
                           arrowprops=dict(arrowstyle='->', color=style['color'], lw=1.5, alpha=0.6))
            ax_obj.annotate('', xy=(x_left, y), xytext=(x, y),
                           arrowprops=dict(arrowstyle='->', color=style['color'], lw=1.5, alpha=0.6))
            
            label = style['label'] if provider not in plotted_providers_ax else None
            ax_obj.scatter(x, y, s=180, marker=style['marker'],
                          color=style['color'], edgecolors='white', linewidth=2,
                          label=label, zorder=3)
            plotted_providers_ax.add(provider)
            
            ox, oy = label_offsets.get(model, (12, 14))
            ax_obj.annotate(model, (x, y), xytext=(ox, oy),
    ...
    # Clean tick labels - use simple values
    ax_left.set_xticks([0.2, 0.3, 0.4, 0.5])
    ax_left.set_xticklabels(['0.2', '0.3', '0.4', '0.5'])
    ax_right.set_xticks([2000, 4000, 6000, 8000, 10000])
    ax_right.set_xticklabels(['2000', '4000', '6000', '8000', '10000'])
    
    # Labels
    ax_left.set_ylabel('Word Error Rate (%)', fontweight='normal', fontsize=13)
    fig.text(0.5, 0.01, 'Latency (ms)', ha='center', fontweight='normal', fontsize=13)
    fig.suptitle('Speech-to-Text Performance', fontweight='bold', fontsize=14, y=0.98)
    
    ax_left.text(0.5, 0.02, 'Local', transform=ax_left.transAxes, fontsize=11,
                color='#555555', fontstyle='italic', ha='center')
    ax_right.text(0.5, 0.02, 'API-Based', transform=ax_right.transAxes, fontsize=11,
                 color='#555555', fontstyle='italic', ha='center')
    ...
    handles_all = []
    labels_all = []
    for p_name in ['aws', 'assemblyai', 'deepgram', 'nvidia', 'whisper']:
        if p_name in [get_provider(m) for m in all_models]:
            s = provider_styles[p_name]
            handles_all.append(Line2D([0], [0], marker=s['marker'], color='w',
                                     markerfacecolor=s['color'], markersize=10))
            labels_all.append(s['label'])
    handles_all.append(Patch(facecolor='gray', alpha=0.2))
    labels_all.append('WER ±1 SD')
    handles_all.append(Line2D([0], [0], color='gray', lw=1.5, alpha=0.6, marker='>', markersize=5))
    labels_all.append('Latency ±1 SD')
    ax_right.legend(handles_all, labels_all, loc='upper left', frameon=True,
...
def main():
    ...
    results_dir = server_dir / "lily-data" / "evaluation_output" / "evaluation"
